...
const handleFileChange = (e) => {
  ...
  files.value.push(...selected) // push files into array
...
const handleDroppedFile = (e) => {
  ...
  const droppedFiles = Array.from(e.dataTransfer.files) // get all files
  if (droppedFiles.length) {
    // mock event object with all files
    handleFileChange({ target: { files: droppedFiles } }) // pass array directly
  }
}
